...
5. Choose `--agent` values only from `agents/roster.yaml` role ids.
6. Enqueue items via `bun run autonomy:enqueue --id <id> --agent <role-id> --priority <pri> --description "<desc>"`.
